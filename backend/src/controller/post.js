...
export const getPost = async (req, res) => {
  try {
    ...
    if (token) {
      ...
      const userLikes = await Like.find({ userId: currentUserId, postId: { $in: postIds } }).select('postId')
      const userBookmarks = await Bookmark.find({ userId: currentUserId, postId: { $in: postIds } }).select('postId')
