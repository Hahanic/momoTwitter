<template>
  <div :class="{'border-b-1': !isCompose, 'h-full max-h-[100vh] flex flex-col': isCompose}" class="w-full dark:border-borderDark border-borderWhite">
      ...
      <div class="text-[#1eaafe] flex items-center gap-4">
        ...
        <div class="block sm:hidden">
          <button
            ...
            :class="{ 'dark:bg-white text-white bg-black hover:cursor-pointer': !!messageContent, 'dark:bg-[#787a7a] bg-[#87898c]': !messageContent }"
    ...
    <div class="flex" :class="{'min-h-0': isCompose}">
      <!-- 头像：适应未登录 -->
      <div>
        <div class="h-[3rem] w-[3rem] mx-2 mt-2 rounded-full flex items-center justify-center">
...
import { Image, LucideBot, MenuIcon, LocationEdit, CalendarClockIcon, SmileIcon, XIcon, ArrowLeft, UserCircle2Icon } from 'lucide-vue-next'
...
const handlePosting = async () => {
  ...
  // 发帖
  try {
    await postStore.createPost({ content: messageContent.value, postType: 'standard' })
